...
conn_str = (
    ...
    "SERVER=sagar;"  # Replace with your server name
...
def upload_file():
    ...
    return render_template('index.html', uploaded_file=file.filename, result_file=os.path.basename(result_path), 
                          person_detected=person_detected, user_email=user_email, notifications=notifications)
...
def process_file(file_path, filename):
    ...
    cursor = conn.cursor()
    cursor.execute("SELECT TOP 1 Email FROM MissingPersonReports ORDER BY SubmittedDate DESC")
...
def send_notification():
    ...
    return render_template('send_notification.html', to_email=user_email, from_email=EMAIL_ADDRESS)
...
def report_missing():
    if request.method == 'POST':
        ...
        cursor.execute("""
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        """, (full_name, address, email, phone, missing_full_name, int(missing_age), pic_path, description))
        conn.commit()
        cursor.close()
        conn.close()

        notifications.append(f"Missing person report submitted for {missing_full_name} by {full_name}")
